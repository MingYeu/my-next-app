...
export const getMembersList = ({ debouncedKeyword, memberId }: { debouncedKeyword: string; memberId?: string }) => {
    return axiosInstance.get<Member[]>(`/api/staff/data/member?keyword=${debouncedKeyword}&perPage=10${memberId ? `&memberId=${memberId}` : ''}`);
};
